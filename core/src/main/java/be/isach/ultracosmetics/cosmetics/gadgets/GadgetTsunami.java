...
import be.isach.ultracosmetics.util.CooldownMap;
...
public class GadgetTsunami extends Gadget implements PlayerAffectingCosmetic {
    ...
    protected void onRightClick() {
        ...
        getUltraCosmetics().getScheduler().runAtLocation(loc, t -> tick(loc, v, 40));
    }
    ...
    private void tick(Location loc, Vector delta, int ticksRemaining) {
        ...
        loc.add(delta);
        if (ticksRemaining > 0) {
            getUltraCosmetics().getScheduler().runAtLocationLater(loc, t -> tick(loc, delta, ticksRemaining - 1), 1);
        }
    }
}
